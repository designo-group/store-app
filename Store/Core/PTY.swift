...
//
//  Adapted from https://github.com/adamcin/RxExecs/blob/master/Sources/RxExecs/PTY.swift
//
import Foundation
import Darwin
...
open class PTY {
    ...
    open func launch() {
        guard let launchPath = self.launchPath else {
            NSLog("**** Error: launchPath must be set before calling launch()")
            return
        }
        self.internalLaunch(launchPath,
                            argv: [launchPath] + (self.arguments ?? []),
                            env: (self.environment ?? ProcessInfo.processInfo.environment).map { "\($0)=\($1)" }
        )
    }
     ...
     */
    /// @discussion
    /// Setting the block to nil is valid, and stops the previous block from being invoked, as long as it hasn't started in any way.
    /// The NSTask is passed as the argument to the block so the block does not have to capture, and thus retain, it.
    /// The block is copied when set. Only one termination handler block can be set at any time.
    /// If the NSTask has already finished, the block is executed immediately/soon (not necessarily on the current thread).
    /// If a terminationHandler is set on an NSTask, the NSTaskDidTerminateNotification notification is not posted for that task.
    /// Also note that -waitUntilExit won't wait until the terminationHandler has been fully executed.
    /// You cannot use this property in a concrete subclass of NSTask which hasn't been updated to include an implementation of the storage and use of it.
    ...
    fileprivate func beginReadingFromPipe(_ stdin: Pipe, master: FileHandle) {
        DispatchQueue.global(qos: self.qosClass).async { [weak self, weak stdin, weak master] () -> Void in
            ...
            let readAgain = data.count > 0 && this.running
            if readAgain {
                master.write(data)
    ...
    fileprivate func beginWritingToPipe(_ master: FileHandle, stdout: Pipe) {
        DispatchQueue.global(qos: self.qosClass).async { [weak self, weak master, weak stdout] () -> Void in
    ...
    fileprivate func internalLaunch(_ path: String, argv: [String], env: [String]) {
        NSLog("%@", "internalLaunch(path: \(path), argv: \(argv), env: \(env))")
    ...
    fileprivate var qosClass: DispatchQoS.QoSClass {
        ...
        default: return DispatchQoS.QoSClass.default
        }
    }
...
//            }
            if s < 0 {
                NSLog("%@", "**** Error in waitid: \(errno)")
                this.terminate(1, reason: .exit)
                return
            }
            var status = stat_loc.si_status
            var reason: Process.TerminationReason = .exit
    ...
    fileprivate func terminate(_ status: Int32, reason: Process.TerminationReason){
        ...
        if let termHandler = self.terminationHandler {
            // NOTE: The execution context in which the block is invoked is undefined,
            // but here it runs on the background queue from waitForProcess.
            termHandler(self)
        }
...
/// Terminating `NULL` is required for `exec~` family calls.
func RxExecWithCPointerToNullTerminatingCArrayOfCStrings(_ strings: [String], block: (UnsafePointer<UnsafeMutablePointer<Int8>?>) -> ()) {
    ...
    let a1: [UnsafeMutablePointer<Int8>?] = strings.map { $0.utf8CString }.map { (d: ContiguousArray<CChar>) in
        ...
        return ptr
    } + [nil] // + Required terminating NULL pointer
    
    // NOTE: Memory leak warning: The allocated memory (`ptr`) is not explicitly deallocated here.
    // In the context of the child process (`RxExecPtyExec`), this is generally acceptable
    // because the process is immediately replaced by `execve`, which cleans up the memory
    // segment. For general-purpose use outside of an exec call, this would require
    // proper deallocation within a `defer` block or after the `block` returns.
...
func RxExecPtyExec(_ path: String, args: [String], env: [String]) {
    path.withCString { (pathP: UnsafePointer<Int8>) -> () in
        ...
        fatalError("`execve` call returned, which indicates a failure, errno: \(errno)")
    }
}
